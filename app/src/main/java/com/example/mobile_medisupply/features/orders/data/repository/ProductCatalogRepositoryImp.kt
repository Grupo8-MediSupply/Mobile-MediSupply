...
class ProductCatalogRepository @Inject constructor(private val api: ProductApi) :
        ...
        try {
            ...
            if (response.success) {
                ...
                emit(
                        Result.failure(
                                Exception(
                                        "Error creando pedido: ${response.message ?: "desconocido"}"
                                )
                        )
                )
            }
    ...
    override suspend fun createOrderByClient(
            products: List<ProductItemRequest>
    ): Flow<Result<OrderCreatedResult>> = flow {
        try {
            val response = api.crearPedidoDesdeCliente(CreateOrderRequest(productos = products))
